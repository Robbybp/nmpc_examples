...
Have looked through KH's MHE example. Looks pretty good.
My main goal now is to make sure my data structures hold up in this
example.
\begin{itemize}
  \item Use them whenever possible in KH's example.
  \item Add functionality that may be useful (e.g. ScalarData class)
    and use it in KH's example (as well as my own)
\end{itemize}
I should not prioritize modifying KH's API for MHE components.
This should be left as comments on KH's PR that he can address as
he sees fit.

\medskip

So how do I go about testing and improving my data structures for KH's example?
\begin{itemize}
  \item \texttt{mhe-patch} branch where I modify KH's original branch to use
    my data structures as much as possible
    \begin{itemize}
      \item This will involve refactoring his test to run faster, so I can actually
        test my modifications.
      \item This will also require a function to create KH's control inputs,
        instead of using the json file.
    \end{itemize}
  \item Make all modifications I can without modifying the data structures.
  \item Make a branch, based on main, to add to data structures.
  \item Add ScalarData class, test in NMPC example.
  \item Test in MHE example (in its own branch, probably)
  \item If satisfied, merge into main, then merge main into KH's MHE
    branch and my patch. Open a PR to KH's branch to use my patch.
\end{itemize}
...
Reviewing KH's code.
\begin{enumerate}
  \item Read code, mark up inline
  \item Refactor test to run quickly
  \item Start editing code.
\end{enumerate}

What are my goals wrt editing KH's code?
\begin{itemize}
  \item Improve the API for MHE?
  \item Improve/extend my own code to make MHE easier/more straightforward?
\end{itemize}
Probably both, but mostly the second.
